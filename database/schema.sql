...
CREATE TABLE IF NOT EXISTS admins (tg_id INTEGER PRIMARY KEY, name TEXT, role TEXT DEFAULT 'admin', permissions TEXT DEFAULT '{}', active INTEGER DEFAULT 1, warning_count INTEGER DEFAULT 0, warning_limit INTEGER DEFAULT 3, dossier TEXT DEFAULT '', created_at TEXT);
CREATE TABLE IF NOT EXISTS admin_warnings (id INTEGER PRIMARY KEY AUTOINCREMENT, admin_tg_id INTEGER, owner_tg_id INTEGER, reason TEXT, created_at TEXT);
...
CREATE TABLE IF NOT EXISTS challenges (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, body TEXT, media_file_id TEXT, channel_id TEXT, starts_at TEXT, ends_at TEXT, status TEXT DEFAULT 'open', template TEXT, created_at TEXT);
...
CREATE TABLE IF NOT EXISTS challenge_votes (entry_id INTEGER, user_tg_id INTEGER, vote TEXT, created_at TEXT, PRIMARY KEY(entry_id,user_tg_id));
...
('force_join','🔐 برای استفاده از این بخش ابتدا در کانال‌ها/گروه‌های زیر عضو شوید و دوباره تلاش کنید.'),
('matchbot_soon','🤖 موتور رل‌یابی هوشمند در نسخه بعدی فعال می‌شود؛ جایگاه دیتابیس و پنل آن آماده است.');
INSERT OR IGNORE INTO categories(type,title,description,sort_order,custom_text) VALUES
('ticket','پشتیبانی','ارسال تیکت برای پشتیبانی',10,''),
('challenge','چالش عکس','عکس خود را برای شرکت در چالش بفرستید.',20,'اکنون می‌توانید در چالش شرکت کنید؛ عکس یا متن خود را ارسال کنید.'),
